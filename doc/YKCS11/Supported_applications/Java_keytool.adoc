...
To sign a jar file using `jarsigner`, the alias of the signing key needs to be specified. The aliases of the keys stored on the YubiKey PIV are fixed and unmodifiable. The key aliases are displayed when listing the content of the YubiKey using `keytool -list` above or they can be found in link:../index.adoc[this list]
